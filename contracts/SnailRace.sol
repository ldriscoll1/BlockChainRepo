...
contract SnailRace{
    ...
    function assignPlayers(address _playerOne, address _playerTwo, uint256 _snailID) public {
        require(_snailID <= snails.length, "Snail ID does not exist");
        require(_snailID > 0, "Snail ID does not exist");
    ...
    function getLeaderboardEntryRaceTime(uint256 _entryID) public view returns(uint256){
        //Require input to be in bounds of the leaderboard array
        require(_entryID < leaderboard.length, "Entry ID is out of bounds");
        return (leaderboard[_entryID].raceTime);
    }
